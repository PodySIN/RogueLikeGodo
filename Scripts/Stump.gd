...
func get_death():
	...
	Stump_GOLD_cost = randi_range(Stump_left_gold,Stump_right_gold)
...
func _on_enemy_hitbox_area_entered(area):
	if area.name == 'Bullet':
		...
		Stump_health -= playershotgun_dmg
...
func _on_main_upgrade_timer_timeout():
	if upgrade_times <= 4:
		...
		Stump_max_health = int(Stump_max_health * 1.1)
		...
		Stump_EXP_cost = int(Stump_EXP_cost * 2)
		Stump_left_gold += 11
		Stump_right_gold += 11
	upgrade_times += 1
